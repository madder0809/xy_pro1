...
class GzucmExcel extends Controller {
    ...
    function getSheet($filename,$sheetName){
        ...
        $this->activeSheet = $PHPReader->getSheetByName($sheetName);
    }

    function getCellValue($map){
        return $this->activeSheet->getCell($map)->getCalculatedValue();
    }

    // 按工作表名来解析数据
    function parseExcelBySheetName($start_row=3){
